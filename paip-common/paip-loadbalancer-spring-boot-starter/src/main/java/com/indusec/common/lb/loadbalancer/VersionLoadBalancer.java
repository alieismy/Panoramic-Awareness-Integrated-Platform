package com.indusec.common.lb.loadbalancer;

import cn.hutool.core.util.StrUtil;
import com.indusec.common.constant.CommonConstant;
import com.indusec.common.lb.chooser.IRuleChooser;
import com.indusec.common.lb.utils.QueryUtils;
...
public class VersionLoadBalancer implements ReactorServiceInstanceLoadBalancer {
    ...
    private String getVersionFromRequestData(RequestData requestData) {
        ...
        if (MapUtils.isNotEmpty(queryMap) && queryMap.containsKey(CommonConstant.PAIP_VERSION) && StrUtil.isNotBlank(queryMap.get(CommonConstant.PAIP_VERSION))) {
            return queryMap.get(CommonConstant.PAIP_VERSION);
        } else if (requestData.getHeaders().containsKey(CommonConstant.PAIP_VERSION)) {
            return requestData.getHeaders().get(CommonConstant.PAIP_VERSION).get(0);
        }
        return null;
    }
